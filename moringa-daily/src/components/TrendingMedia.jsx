...
const TrendingMedia = () => {
  ...
  useEffect(() => {
    async function fetchTrendingMedia() {
      try {
        const response = await api.get("/content/random"); // Replace with the actual endpoint
    ...
    <div className='text-white p-12 grid grid-cols-2'>
      ...
      <div className='col-span-1 border rounded p-12 flex items-center justify-center bg-slate-200'>
        <iframe
          width='350'
          height='200'
          src={trendingMedia?.video_url || "https://www.youtube.com/embed/your-default-video"}
